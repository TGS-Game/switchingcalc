...
class RawStatementRow(db.Model):
    ...
    row_signature = db.Column(db.String(64), index=True)
    normalized = db.Column(db.Boolean, default=False, nullable=False)


class SuggestedTransaction(db.Model):
    __tablename__ = "suggested_transactions"
    id = db.Column(UUID(as_uuid=True), primary_key=True, default=gen_uuid)
    user_id = db.Column(UUID(as_uuid=True), db.ForeignKey("users.id"), nullable=False, index=True)
    upload_batch_id = db.Column(UUID(as_uuid=True), db.ForeignKey("upload_batches.id"), nullable=False, index=True)
    source_row_id = db.Column(UUID(as_uuid=True), db.ForeignKey("raw_statement_rows.id"), nullable=False)
    paired_row_id = db.Column(UUID(as_uuid=True), db.ForeignKey("raw_statement_rows.id"))
    group_key = db.Column(db.String(255), index=True)
    suggestion_type = db.Column(db.String(32), nullable=False, index=True)
    metal = db.Column(db.String(32))
    from_metal = db.Column(db.String(32))
    to_metal = db.Column(db.String(32))
    quantity_grams = db.Column(db.Numeric(18, 6))
    to_quantity_grams = db.Column(db.Numeric(18, 6))
    ratio = db.Column(db.Numeric(18, 6))
    notes = db.Column(db.Text)
    confidence = db.Column(db.Numeric(5, 2), nullable=False, default=0)
    review_status = db.Column(db.String(32), nullable=False, default="pending", index=True)
    amended_type = db.Column(db.String(32))
    amended_metal = db.Column(db.String(32))
    amended_from_metal = db.Column(db.String(32))
    amended_to_metal = db.Column(db.String(32))
    amended_quantity_grams = db.Column(db.Numeric(18, 6))
    amended_to_quantity_grams = db.Column(db.Numeric(18, 6))
    amended_ratio = db.Column(db.Numeric(18, 6))
    amended_notes = db.Column(db.Text)
    confirmed_transaction_id = db.Column(UUID(as_uuid=True))
    created_at = db.Column(db.DateTime, default=datetime.utcnow, nullable=False)
...
class Transaction(db.Model):
    ...
    source_suggestion_id = db.Column(UUID(as_uuid=True), index=True)
    ...
    metal = db.Column(db.Enum(MetalType), nullable=True)
    from_metal = db.Column(db.Enum(MetalType), nullable=True)
    to_metal = db.Column(db.Enum(MetalType), nullable=True)
...
class SavedCalculationScenario(db.Model):
    __tablename__ = "saved_calculation_scenarios"
    id = db.Column(UUID(as_uuid=True), primary_key=True, default=gen_uuid)
    user_id = db.Column(UUID(as_uuid=True), db.ForeignKey("users.id"), nullable=False, index=True)
    scenario_name = db.Column(db.String(255))
    notes = db.Column(db.Text)
    future_ratio = db.Column(db.Numeric(18, 6), nullable=False)
    fee_percent = db.Column(db.Numeric(5, 2), nullable=False, default=3)
    snapshot_json = db.Column(JSONB, default=dict, nullable=False)

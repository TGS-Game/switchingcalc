...
function LoginScreen({ onLogin }) {
  ...
  async function handleSubmit(e) {
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error(data.message || data.msg || `Login failed (${response.status})`);
      }
      ...
      <div style={{ display: 'flex', minHeight: '100vh', background: '#e5e7eb' }}>
        <aside style={{
          ...
          <h1 style={{
            ...
            {collapsed ? 'MT' : 'Metals Tracker'}
          </h1>

          <nav style={{ display: 'grid', gap: 8 }}>
            <NavItem to="/" label="Current Portfolio" icon={'\u{1F4D8}'} collapsed={collapsed} />
            <NavItem to="/upload" label="Upload & Assign" icon={'\u{2B06}\u{FE0F}'} collapsed={collapsed} />
            <NavItem to="/settings" label="Settings" icon={'\u2699\uFE0F'} collapsed={collapsed} />
          </nav>

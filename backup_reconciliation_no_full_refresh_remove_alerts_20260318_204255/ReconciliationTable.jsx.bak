...
export default function ReconciliationTable({ cases, refresh, currentStatus, onProcessed }) {
  ...
  const [expandedDefinitionIds, setExpandedDefinitionIds] = useState({});
  ...
  const resolve = async (item) => {
    setSavingId(item.id);
    try {
      const response = await client.post(`/reconciliation/${item.id}/resolve`, getDraft(item));
      const result = response.data;

      if (currentStatus === 'pending' && ['confirmed', 'amended', 'ignored', 'rejected'].includes(result.review_status)) {
        onProcessed?.([item.id]);
      }
  ...
  const toggleDefinition = (id) => {
    setExpandedDefinitionIds((prev) => ({ ...prev, [id]: !prev[id] }));
  };
            ...
            {orderedCases.map((item) => {
              ...
              return (
                <tr key={item.id} style={{ background: isSelected ? '#f8fafc' : '#ffffff' }}>
                  ...
                  <td style={{ ...cellStyle, width: 210, maxWidth: 210 }}>
                  ...
                  <td style={cellStyle}>
                    {isSwitch ? (
                      ...
                      <div style={{ display: 'grid', gap: 3 }}>
                        <div style={{ fontSize: 11, color: '#475569' }}>Received</div>
                        <input
                          type="number"
                          step="0.000001"
                      ...
                      <div style={{ display: 'grid', gap: 3 }}>
                        <div style={{ fontSize: 11, color: '#475569' }}>Sent</div>
                        <input
                          type="number"
                          step="0.000001"
                          value={draft.to_quantity_grams}
